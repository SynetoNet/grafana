define([
	...
	function (angular, $, kbn, _, config, moment, Modernizr) {
		...
		module.service('dashboard', function ($routeParams, $http, $rootScope, $injector, $location, $timeout,
			...
			var route = function () {
				$location.path('/dashboard/file/' + $('#current-page').val() + '.json');
			...
			this.to_server = function () {
				...
				return $http({
					...
					if (!result) {
						alertSrv.set('Error', "Could not save dashboard.", 'error');
						return false;
					}
					alertSrv.set('Success', "Dashboard saved.", 'success');
					return true;
				}, function () {
					alertSrv.set('Error', "Could not save dashboard.", 'error');
					return false;
			...
			this.file_load = function (file) {
				return $http({
					url: "/grafana/app/dashboards/" + file.replace(/\.(?!json)/, "/") + '?' + new Date().getTime(),
					method: "GET",
			...
			this.script_load = function (file) {
				return $http({
					url: "/grafana/app/dashboards/" + file.replace(/\.(?!js)/, "/"),
					method: "GET"
				})
					.then(function (result) {
						/*jshint -W054 */
